...
import { PlatformInfo, ToolCall } from "../types/index.js";
import { PermissionDecision } from "../types/index.js";
    ...
    try {
      ...
      if (this.platform.os === "windows") {
        ...
        if (shell.toLowerCase().includes("powershell") || shell.toLowerCase().includes("pwsh")) {
          execSync(`cmd /c "where ${command} 2>nul"`, opts);
        } else {
          execSync(`where ${command} 2>nul`, opts);
        }
      } else {
        execSync(`which ${command} 2>/dev/null`, opts);
      }
      return true;
    } catch {
      return false;
