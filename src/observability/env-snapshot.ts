...
const snapshotCache = new Map<string, { snapshot: EnvSnapshot; timestamp: number }>();
...
export function captureEnvSnapshot(
  ...
  const cached = snapshotCache.get(projectPath);
  if (cached && Date.now() - cached.timestamp < SNAPSHOT_CACHE_TTL) {
    return cached.snapshot;
  }

  const snapshot = captureEnvSnapshotUncached(projectPath, platform);

  snapshotCache.set(projectPath, { snapshot, timestamp: Date.now() });
  return snapshot;
}

export function invalidateEnvSnapshotCache(projectPath?: string): void {
  if (projectPath) {
    snapshotCache.delete(projectPath);
  } else {
    snapshotCache.clear();
  }
}

function captureEnvSnapshotUncached(
  projectPath: string,
  platform: PlatformInfo
): EnvSnapshot {
  try {
