...
// =============================================================================
// Adapted imports — originally from Claude Code internals
// =============================================================================

/** Local replacement for `import { env } from '../../utils/env.js'` */
const env = {
  terminal: process.env['TERM'] ?? '',
}

/**
 * Local replacement for `import { execFileNoThrow } from '../../utils/execFileNoThrow.js'`.
 * Runs a child process and returns { code } without throwing on non-zero exit.
 */
...
async function execFileNoThrow(
  ...
  try {
    await execFileAsync(file, args, {
      ...
      ...(options?.input != null ? { input: options.input } as any : {}),
    })
    return { code: 0 }
  } catch (err: any) {
    return { code: err.code ?? 1 }
  }
}

// =============================================================================
// Original osc.ts content follows (parsing logic unchanged)
// =============================================================================
