/**
 * Bash tokenizer / lexer types and utilities.
 *
 * Core types (TsNode, Token, Lexer), character classification helpers,
 * and the bash lexer (makeLexer, nextToken, skipBlanks).
 * All other parser modules import their base types from here.
 *
 * Reference: tree-sitter-bash grammar. Validated against a 3449-input
 * golden corpus generated from the WASM parser.
 */

import { parseSource } from './bash-parser-core.js'
...
export const MODULE: ParserModule = { parse: parseSource }
...
/** Sentinel returned by parseCommandRaw when the parse is aborted. */
export const PARSE_ABORTED = Symbol('PARSE_ABORTED')

/**
 * Parse a single command string, returning the TsNode tree, null if the
 * parser module is not loaded, or PARSE_ABORTED if parsing timed out or
 * exceeded the node budget.
 */
export async function parseCommandRaw(
  cmd: string,
): Promise<TsNode | null | typeof PARSE_ABORTED> {
  const mod = getParserModule()
  if (!mod) return null
  let result: TsNode | null = null
  try {
    result = mod.parse(cmd)
  } catch {
    return null
  }
  return result === null ? PARSE_ABORTED : result
}
