...
const CATEGORY_MAP: Record<string, { fileType: string; category: string }> = {
  ...
  ".env": { fileType: "configuration", category: "env_config" },
};

const MODULE_PURPOSES: Record<string, { purpose: string; description: string }> = {
  ...
  ".github/": { purpose: "GitHub configuration", description: "CI/CD workflows, issue templates, and community templates" },
  ".husky/": { purpose: "Git hooks", description: "Pre-commit and commit-msg hooks for code quality enforcement" },
};

function getFilePurpose(filePath: string): { purpose: string; description: string } {
...
function detectQualityIssues(filePath: string, lines: number, ext: string): string[] {
  const issues: string[] = [];
  if (ext === ".ts" || ext === ".tsx") {
    if (lines > 500) issues.push("LARGE_FILE: exceeds 500 LOC, consider splitting");
    if (lines > 1000) issues.push("VERY_LARGE_FILE: exceeds 1000 LOC, high priority for modularization");
  }
  if (filePath.includes("test/") && (ext === ".ts" || ext === ".tsx")) {
    if (lines > 800) issues.push("LARGE_TEST_FILE: consider splitting test suite");
  }
  if (filePath.endsWith("index.ts") && lines > 100) {
    issues.push("BARREL_FILE_LARGE: barrel export file exceeds 100 lines");
  }
  return issues;
}

function main() {
  const inventoryRaw = JSON.parse(fs.readFileSync("project-docs/file-inventory-raw.json", "utf-8")) as FileEntry[];
  const depRaw = JSON.parse(fs.readFileSync("project-docs/dependency-raw.json", "utf-8")) as DepEntry[];
  ...
  const summary = {
    ...
    qualityIssueTypes: catalog.flatMap((c) => c.qualityIssues).reduce<Record<string, number>>((acc, issue) => {
  ...
  console.log(`Catalog generated: ${catalog.length} files, ${totalLOC} total LOC`);
  console.log(`Files with quality issues: ${summary.filesWithQualityIssues}`);
